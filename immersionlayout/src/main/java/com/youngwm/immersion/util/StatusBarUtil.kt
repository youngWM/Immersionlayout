...
object StatusBarUtil {
    ...
    public fun toolbarGradientWhenScrollBase(scrollView: NestedScrollView,
                                      ...
                                      @ColorInt toolbarBgColor: Int,
                                      toolbar: View,
                                      toolbarChildShowMode: MutableMap<View, ImmersionScrollRefreshLayout.ImmersionShowParam>){
        scrollView.setOnScrollChangeListener(object : NestedScrollView.OnScrollChangeListener {

            private var mScrollY = 0
            private var mLastScrollY = 0
            private val opacityDistance: Int = dp2px(scrollHeight)
            private val mColor = toolbarBgColor and 0x00ffffff

            override fun onScrollChange(v: NestedScrollView, scrollX: Int, scrollY: Int, oldScrollX: Int, oldScrollY: Int) {
                var scrollY = scrollY
                if (mLastScrollY <= this.opacityDistance) {
                    scrollY = Math.min(this.opacityDistance, scrollY)
                    mScrollY = Math.min(scrollY, this.opacityDistance)
                    val viewAlpha = 1f * mScrollY / opacityDistance
                    toolbarChildShowMode.forEach {
                        val view = it.key
                        val immersionShowParam = it.value

                        if (view is TextView && immersionShowParam.textColorBefore != -1){}
                        it.key.apply { alpha = viewAlpha }
                    }
                    toolbar.setBackgroundColor(255 * mScrollY / this.opacityDistance shl 24 or mColor)
                }else{
